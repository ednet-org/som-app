...
class Concept extends Entity<Concept> {
  ...
  Concept(this.model, String conceptCode)
    ...
    // Concept entities are self-referential - their concept is themselves
    _concept = this;
  }
  ...
  Map<String, dynamic> toGraph() {
    ...
    graph['min'] = min;
    graph['max'] = max;
    graph['updateOid'] = updateOid;
    graph['updateCode'] = updateCode;
    graph['updateWhen'] = updateWhen;
    graph['attributes'] = attributes
        .toList()
        .map((attribute) => attribute.toGraph())
        .toList();
    graph['parents'] = parents
        .toList()
        .map((parent) => parent.toGraph())
        .toList();
    graph['children'] = children
        .toList()
        .map((child) => child.toGraph())
        .toList();
    return graph;
  }
  ...
  /// Get concept name (alias for code)
