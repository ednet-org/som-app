...
class SerbianElectionDomain extends DomainModels {
  ...
  List<IzbornaLista> raspodelaMandataDontovomMetodom(
    ...
    kalkulator.izracunajMandate(rezultati, ukupnoMandata, cenzus: cenzus);
  ...
  IzborniZakon createIzborniZakon({
    ...
    final izborniZakonConcept = model.concepts.singleWhereCode(
      SerbianElectionModel.IZBORNI_ZAKON,
    );
  ...
  IzbornaKomisija createIzbornaKomisija({
    ...
    final model = entries.model;
    final izbornaKomisijaConcept = model.concepts.singleWhereCode(
      SerbianElectionModel.IZBORNA_KOMISIJA,
    );
  ...
  TipIzbora createTipIzbora({
    ...
    // Get the concept directly from the model
    final model = entries.model;
    final tipIzboraConcept = model.concepts.singleWhereCode(
      SerbianElectionModel.TIP_IZBORA,
    );
  ...
  IzborniSistem createIzborniSistem({
    ...
    // Get the concept directly from the model
    final model = entries.model;
    final izborniSistemConcept = model.concepts.singleWhereCode(
      SerbianElectionModel.IZBORNI_SISTEM,
    );
...
class IzboriAggregateRoot implements Entity<Izbori> {
  ...
  ValidationExceptions validateRelationships() {
    ...
    for (var glas in _glasovi) {
      if (glas.getParent(SerbianElectionModel.IZBORI_REL) == null) {
        exceptions.add(
          ValidationException('required', 'Glas.izbori parent is required'),
        );
      }
    }
