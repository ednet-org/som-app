...
final Logger _pastLogger = Logger('ednet_core.commands.past');
...
class Past implements IPast {
  ...
  void display([String title = 'Past Commands', StringSink? out]) {
    ...
    for (IBasicCommand command in commands) {
      command.display(out: sink);
    }
    sink.writeln('');

    if (out == null) {
      _pastLogger.fine(sink.toString());
    }
  }
}
