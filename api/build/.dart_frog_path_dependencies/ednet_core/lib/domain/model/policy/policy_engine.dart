...
class PolicyEngine implements IPolicyEngine {
  ...
  void addPolicy(IPolicy policy) {
    // Cast to Policy for backward compatibility with existing code
    if (policy is Policy) {
      _policies.add(policy);
    } else {
      throw ArgumentError(
        'PolicyEngine currently only supports Policy instances',
      );
    }
  }
  ...
  @override
  List<IPolicy> get policies => List.unmodifiable(_policies.cast<IPolicy>());

  @override
  List<IPolicy> getApplicablePolicies(Entity entity) {
    return _policies
        .where((policy) => policy.evaluate(entity))
        .cast<IPolicy>()
        .toList();
  }

  /// Legacy method for backward compatibility
  /// Gets all policies that apply to the given entity as Policy objects.
  List<Policy> getApplicablePoliciesLegacy(Entity entity) {
  ...
  List<dynamic> executePolicies(Entity entity) {
    ...
    final applicablePolicies = getApplicablePoliciesLegacy(entity);
