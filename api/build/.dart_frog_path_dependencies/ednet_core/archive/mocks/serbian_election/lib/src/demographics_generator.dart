...
class SrbijaDemografskiGenerator {
  ...
  Glasac _generisiGlasaca({IzbornaJedinica? birackoMesto}) {
    ...
    final region = _izaberiNaOsnovuRaspodele(
      _raspodele.raspodelaPoBrojuStanovnika,
    );
  ...
  String _generisiJMBG(
    ...
    // Regionalni kod
    final regionKod =
        _raspodele.regionskiKodovi[region]?[opstina] ??
    ...
    final kontrolniKarakter = kontrolnaCifra > 9
        ? '0'
        : kontrolnaCifra.toString();
  ...
  DateTime _generisiDatumRodjenja() {
    ...
    final starosnaGrupa = _izaberiNaOsnovuRaspodele(
      _raspodele.raspodelaPogodinama,
    );
    ...
    return DateTime(danas.year - godine, meseci, dani);
  }
  ...
  void simulirajGlasanje(
    ...
    double ukupanProcenat = raspodelaGlasova.values.fold(
      0,
      (sum, value) => sum + value,
    );
