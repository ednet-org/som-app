...
final Logger _entitiesLogger = Logger('ednet_core.entities');
...
class Entities<E extends Entity<E>> implements IEntities<E> {
  ...
  Entity<E> newEntity() {
    // Special handling for DynamicEntity to ensure proper type
    if (E == DynamicEntity) {
      final dynamicEntity = DynamicEntity();
      dynamicEntity.concept = _concept!;
      return dynamicEntity as Entity<E>;
    }
  ...
  Entities<E> selectWhereAttribute(String code, Object attribute) {
    ...
    for (E entity in _entityList) {
      if (entity.getAttribute(code) == attribute) {
        selectedEntities.add(entity);
      }
    }
    selectedEntities.pre = true;
  ...
  Entities<E> selectWhereParent(String code, IEntity parent) {
    ...
    selectedEntities.propagateToSource = false;
    for (E entity in _entityList) {
      if (entity.getParent(code) == parent) {
        selectedEntities.add(entity);
      }
    }
    selectedEntities.pre = true;
  ...
  String toString() {
    if (_concept == null) {
      throw ConceptException('Entities.toString: concept is not defined.');
    }
  ...
  bool isValid(E entity) {
    ...
    if (!_concept!.canAdd) {
      throw AddException('An entity cannot be added to ${_concept!.code}.');
    }
  ...
  bool validateIncrementAndRequired(entity, bool isValid) {
    ...
    for (Attribute a in _concept!.attributes.whereType<Attribute>()) {
      ...
      if (shouldIncrement) {
        ...
        // Type validation - all types should now be properly registered
        // in the domain type system (including generic types like List<String>)
        if (a.type == null) {
          // Type not registered - this indicates a configuration issue
          // Log warning but allow the value (graceful degradation)
          // In production, all types should be registered during YAML loading
          // or via generated _initTypes() method
          continue;
        }

        // Full type validation including collection element validation
  ...
  bool update(E beforeEntity, E afterEntity) {
    if (_concept == null) {
      throw ConceptException('Entities.update: concept is not defined.');
    }
    ...
    if (remove(beforeEntity)) {
      if (add(afterEntity)) {
        ...
        _entitiesLogger.warning(
          'Entities.update failed: ${exceptions.toList()}',
        );
        if (add(beforeEntity)) {
          const category = 'update';
          final message =
  ...
  void display({
    ...
    if (_concept == null) {
      throw ConceptException('Entities.display: concept is not defined.');
    }
    ...
    if (out == null) {
      _entitiesLogger.fine(sink.toString());
    }
  }

  /// Displays the OID map.
  void displayOidMap({StringSink? out}) {
    final sink = out ?? StringBuffer();
    _oidEntityMap.forEach((k, v) {
      sink.writeln('oid $k: $v');
    });
  ...
  void displayCodeMap({StringSink? out}) {
    final sink = out ?? StringBuffer();
    _codeEntityMap.forEach((k, v) {
      sink.writeln('code $k: $v');
    });
  ...
  /// Displays the ID map.
  void displayIdMap({StringSink? out}) {
    final sink = out ?? StringBuffer();
