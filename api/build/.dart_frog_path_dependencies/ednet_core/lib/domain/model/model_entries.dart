...
final Logger _modelEntriesLogger = Logger('ednet_core.model_entries');
...
class ModelEntries implements IModelEntries {
  ...
  Map<String, Entities> newEntries() {
    ...
    for (var entryConcept in _model.entryConcepts) {
      // Create Entities<DynamicEntity> for runtime entity support
      // This allows runtime-created entities without predefined classes
      Entities? entryEntities = Entities<DynamicEntity>();
  ...
  Entities? newEntities(String conceptCode) {
    ...
    if (concept == null) {
      throw ConceptException('${conceptCode} concept does not exist.');
    }

    if (!concept.entry) {
      // Use DynamicEntity for runtime entity support
      var entities = Entities<DynamicEntity>();
  ...
  Entity? newEntity(String conceptCode) {
    ...
    // Create DynamicEntity for runtime entity support
    var conceptEntity = DynamicEntity();
  ...
  ///
  /// [entryConceptCode] is the code identifying the entry concept.
  void displayEntryJson(String entryConceptCode, {StringSink? out}) {
    ...
    sink
      ...
      ..writeln(fromEntryToJson(entryConceptCode))
      ..writeln(
        '--------------------------------------------------------------',
      )
      ..writeln('');
    ...
    if (out == null) {
      _modelEntriesLogger.fine(sink.toString());
    }
  }

  /// Displays the JSON representation of all entries.
  void displayJson({StringSink? out}) {
    final sink = out ?? StringBuffer();
    sink
      ..writeln(
        '==============================================================',
      )
      ..writeln('${_model.domain.code} ${_model.code} Data in JSON')
      ..writeln(
        '==============================================================',
      )
      ..writeln(toJson())
