...
abstract class IDomainEvent {
  ...
  /// Gets the event data as a map, excluding metadata fields.
  ///
  /// This method provides type-safe access to event payload data
...
class DomainEvent implements IDomainEvent {
  ...
  Event toBaseEvent() {
    ...
    // Convert all values for the Event data
    jsonMap.forEach((key, value) {
      if (key != 'entity') {
        // Skip entity which is handled specially
        data[key] = value;
      }
    });
...
IDomainEvent domainEventFromBaseEvent(Event baseEvent, {Entity? entity}) {
  return DomainEvent(
    name: baseEvent.name,
    entity: entity ?? baseEvent.entity,
    aggregateId: baseEvent.data['aggregateId']?.toString() ?? '',
    aggregateType: baseEvent.data['aggregateType']?.toString() ?? '',
    aggregateVersion:
        int.tryParse(baseEvent.data['aggregateVersion']?.toString() ?? '0') ??
        0,
  );
}
