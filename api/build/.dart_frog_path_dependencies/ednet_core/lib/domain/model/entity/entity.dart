...
final Logger _entityLogger = Logger('ednet_core.entity');
...
class Entity<E extends Entity<E>> implements IEntity<E> {
  ...
  /// Creates a new instance of the concrete entity type.
  ///
  /// Subclasses should override this method to return the correct concrete type.
  /// This is used by [copy] to ensure the copied entity is of the correct type.
  ///
  /// Example:
  /// ```dart
  /// class Member extends Entity<Member> {
  ///   @override
  ///   Member createInstance() => Member(concept!);
  /// }
  /// ```
  E createInstance() {
    // Default implementation delegates to newEntity() and attempts cast.
    // Subclasses should override this to return the correct concrete type.
  ...
  E copy() {
    ...
    // Use createInstance() to get correct concrete type (subclasses override this)
  ...
  /// Prints [toString()].
  void displayToString({StringSink? out}) {
    final sink = out ?? StringBuffer();
    sink.writeln(toString());

    if (out == null) {
      _entityLogger.fine(sink.toString());
    }
  }
  ...
  void display({
    ...
    sink.writeln('$s------------------------------------');
    sink.writeln('$s${toString()}                       ');
    sink.writeln('$s------------------------------------');
    s = '$s  ';
    if (withOid) {
      sink.writeln('${s}oid: $_oid');
    }
    sink.writeln('${s}code: $_code');
    sink.writeln('${s}id: $id');
    sink.writeln('${s}whenAdded: $_whenAdded');
    sink.writeln('${s}whenSet: $_whenSet');
    sink.writeln('${s}whenRemoved: $_whenRemoved');
  ...
  Map<String, dynamic> toJsonMap() {
    ...
    for (Attribute attribute in _concept!.attributes.whereType<Attribute>()) {
      ...
      if (value != null) {
        ...
        if (attribute.type?.code == 'DateTime' && value is DateTime) {
          attributeMap[attribute.code] = value.toIso8601String();
        } else {
    ...
    for (Child child in _concept!.children.whereType<Child>()) {
      ...
      if (childEntities != null) {
        childMap[child.code] = (getInternalChild(child.code) as Entities)
            .toJsonList();
      }
    }
  ...
  void fromJsonMap(Map<String, dynamic> entityMap) {
    // Load basic properties - use copy() pattern to temporarily enable updates
    var oidStr = entityMap['oid'];
    try {
      int timeStamp = int.parse(oidStr);
      // Temporarily enable OID updates (same pattern as copy())
      var beforeUpdateOid = _concept?.updateOid ?? false;
      if (_concept != null) _concept!.updateOid = true;
      oid = Oid.ts(timeStamp);
      if (_concept != null) _concept!.updateOid = beforeUpdateOid;
    } catch (e) {
    ...
    if (entityMap.containsKey('code')) {
      // Temporarily enable code updates (same pattern as copy())
      var beforeUpdateCode = _concept?.updateCode ?? false;
      if (_concept != null) _concept!.updateCode = true;
      code = entityMap['code'] as String?;
      if (_concept != null) _concept!.updateCode = beforeUpdateCode;
    }
    ...
    if (entityMap.containsKey('attributes')) {
      ...
      for (Attribute attribute in _concept!.attributes.whereType<Attribute>()) {
        if (attributeMap.containsKey(attribute.code)) {
          var value = attributeMap[attribute.code];
          // Parse ISO 8601 strings back to DateTime
          if (attribute.type?.code == 'DateTime' && value is String) {
            value = DateTime.parse(value);
          }
          setAttribute(attribute.code, value);
        }
      }
    }
    ...
    if (entityMap.containsKey('parents')) {
      ...
      for (Parent parent in _concept!.parents.whereType<Parent>()) {
        if (parentMap.containsKey(parent.code)) {
          ...
          // Oid parentOid = reference.oid; // NOTE: Use parentOid for validation if needed
