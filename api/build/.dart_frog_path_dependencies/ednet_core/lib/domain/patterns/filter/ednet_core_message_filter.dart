...
class FilterEntity extends Entity<FilterEntity> {
  /// Creates a filter entity with optional concept
  FilterEntity({Concept? concept}) : super() {
    if (concept != null) {
  ...
  /// Gets the filter operation (for composite filters)
  String? get operation => getAttribute('operation');

  /// Gets the filter count (for composite filters)
  int? get filterCount => getAttribute('filterCount') as int?;

  /// Gets the selector type (for selector filters)
  String? get selectorType => getAttribute('selectorType');

  /// Checks if the entity is empty
  bool get isEmpty => filterName == null && filterType == null;

  /// Creates a new instance of FilterEntity
  @override
  FilterEntity newEntity() {
    var entity = FilterEntity(concept: concept);
    return entity;
  }
}
...
class MessageFilterRepository {
  ...
  ///
  /// Returns null if no filter with the given name exists or reconstruction fails
  Future<EDNetCoreMessageFilter?> getFilter(String name) async {
    // Find entity by name attribute
    for (var entity in entities) {
      if (entity.filterName == name) {
        try {
          // Create source and target channels (these would typically be provided or looked up)
          // For now, create default in-memory channels
          final sourceChannel = InMemoryChannel(id: 'source-$name');
          final targetChannel = InMemoryChannel(id: 'target-$name');

          // Reconstruct filter based on type
          MessageFilter? filterImpl;
          final filterType = entity.filterType;

          if (filterType == 'predicate') {
            // For predicate filters, we need to reconstruct the predicate function
            // This is a limitation - we can't serialize functions, so we'd need
            // a predicate registry or factory pattern in a real implementation
            filterImpl = null; // Cannot reconstruct predicate function
          } else if (filterType == 'selector') {
            // Similar limitation for selector functions
            filterImpl = null; // Cannot reconstruct selector function
          } else if (filterType == 'composite') {
            // Similar limitation for composite predicates
            filterImpl = null; // Cannot reconstruct composite predicates
          }
