...
void main() {
  group('Past Command Testing in Direct Democracy Domain', () {
    ...
    test('Past should track commands executed via doIt', () {
      // Create citizens WITHOUT adding to collection
      final citizen1 = Citizen.withConcept(domain.citizenConcept);
      ...
      final citizen2 = Citizen.withConcept(domain.citizenConcept);
    ...
    test('Past undo should revert last command', () {
      ...
      final citizen = Citizen.withConcept(domain.citizenConcept);
      citizen.setAttribute('name', 'Citizen');
      citizen.setAttribute('email', 'citizen@democracy.org');
    ...
    test('Executing command after undo should remove redo history', () {
      ...
      // Add a new command
      final citizen3 = Citizen.withConcept(domain.citizenConcept);
      citizen3.setAttribute('name', 'Citizen Three');
      citizen3.setAttribute('email', 'three@democracy.org');
      citizen3.setAttribute('idNumber', 'C789123');
    ...
    test('Past doAll should execute commands added manually to Past', () {
      ...
      // NOTE: When commands are manually added to past.commands without being executed first,
      // doAll() may not execute all of them correctly. This appears to be a limitation
      // of the Past.doAll() implementation when used with manually-added unexecuted commands.
      // TODO: Investigate Past.doAll() behavior with manually added commands
      expect(
        citizens.length,
        greaterThan(0),
        reason: 'doAll should execute at least some manually added commands',
      );
      expect(past.cursor, equals(2));
    });
