...
void main() async {
  const coreFilePath = 'lib/ednet_core.dart';
  const logFilePath = 'missing-parts.log';
  const fixSuggestionFilePath = 'part-fixes.md';
  ...
  try {
    ...
    for (String line in lines) {
      ...
      if (match != null) {
        ...
        if (!await partFile.exists()) {
          ...
          final RegExp partOfRegex = RegExp(
            r'^\s*part\s+of\s+[^;]+;',
            multiLine: true,
          );
    ...
    if (existingPartsWithoutPartOf.isNotEmpty) {
      fixContent.writeln(
        '## Option 3: Fix Existing Files Without "part of" Declarations',
      );
      fixContent.writeln('');
      fixContent.writeln(
        'Add "part of ednet_core;" to the beginning of these files:',
      );
      fixContent.writeln('');
    ...
    print(
      'Total files lacking "part of" declaration: ${existingPartsWithoutPartOf.length}',
    );
  } catch (e) {
    print('Error: $e');
  }
