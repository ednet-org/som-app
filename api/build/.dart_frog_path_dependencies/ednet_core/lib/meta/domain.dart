...
class Domain extends Entity<Domain> {
  ...
  /// Convert domain to Map representation
  Map<String, dynamic> toMap() {
    return {
      ...
      'models': models
          .toList()
          .map(
            (model) => {'code': model.code, 'description': model.description},
          )
          .toList(),
    };
  }

  /// Create domain from Map representation
  static Domain fromMap(Map<String, dynamic> map) {
    final domain = Domain(map['code'] ?? 'default');
    domain.description = map['description'] ?? 'No description';
    return domain;
  }
