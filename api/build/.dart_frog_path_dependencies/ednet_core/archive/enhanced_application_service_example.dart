...
Future<CommandEventPolicyInfrastructure>
setupCommandEventPolicyInfrastructure() async {
  ...
  eventBus.registerHandler<InventoryReservedEvent>(
...
Future<void> demonstrateBasicCommandExecution(
  CommandEventPolicyInfrastructure infra,
) async {
  ...
  final result = await infra.applicationService.executeCommand(
    createOrderCommand,
  );
...
Future<void> demonstrateWorkflowOrchestration(
  CommandEventPolicyInfrastructure infra,
) async {
  ...
  final results = await infra.applicationService.executeWorkflow(
    workflowCommands,
  );
  ...
  for (int i = 0; i < results.length; i++) {
    ...
    print(
      '   Step ${i + 1}: ${result.isSuccess ? '✅ Success' : '❌ Failed'} - ${result.data ?? result.errorMessage}',
    );
  }
...
Future<void> demonstrateErrorHandlingAndRecovery(
  CommandEventPolicyInfrastructure infra,
) async {
...
Future<void> demonstratePerformanceMonitoring(
  CommandEventPolicyInfrastructure infra,
) async {
  ...
  final result = await infra.applicationService.executeCommandWithMetrics(
    monitoredCommand,
  );
...
class Order extends AggregateRoot<Order> {
  ...
  CommandResult processCreateOrder(CreateOrderCommand command) {
    ...
    return CommandResult.success(
      data: {'orderId': oid.toString(), 'total': total},
    );
  }
...
class InventoryCommandHandler
  ...
  Future<CommandResult> handle(ReserveInventoryCommand command) async {
    ...
    return CommandResult.success(
      data: {'orderId': command.orderId, 'reservedItems': command.items.length},
    );
  }
...
class InventoryNotificationHandler
    implements IEventHandler<InventoryReservedEvent> {
  @override
...
class InventoryReservationPolicy implements IEventTriggeredPolicy {
  ...
  void executeActions(Entity entity, dynamic event) {
    print(
      '🔄 Inventory reservation policy triggered for order: ${event.aggregateId}',
    );
  }
  ...
  List<ICommandBusCommand> generateCommands(Entity entity, dynamic event) {
    if (event is OrderCreatedEvent) {
      return [
        ReserveInventoryCommand(orderId: event.aggregateId, items: event.items),
      ];
    }
    return [];
  }
