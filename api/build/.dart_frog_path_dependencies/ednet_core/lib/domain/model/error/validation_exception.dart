...
final Logger _validationExceptionLogger = Logger(
  'ednet_core.validation_exception',
);
...
class ValidationException implements Exception, IValidationExceptions {
  ...
  ValidationException(
    ...
    if (logExceptions) {
      _validationExceptionLogger.warning(
  ...
  static bool logExceptions = false;
  ...
  void display({String prefix = ''}) {
    final buffer = StringBuffer()
      ..writeln('$prefix******************************************')
      ..writeln('$prefix$category                               ')
      ..writeln('$prefix******************************************')
      ..writeln('${prefix}message: $message');
    if (attribute != null) {
      buffer.writeln('${prefix}attribute: $attribute');
    }
    if (entity != null) {
      buffer.writeln('${prefix}entity: ${entity.toString()}');
    }
    buffer
      ..writeln('$prefix******************************************')
      ..writeln('');

    _validationExceptionLogger.fine(buffer.toString());
  }
