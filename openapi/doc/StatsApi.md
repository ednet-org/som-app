...
# **statsConsultantGet**
> BuiltMap<String, int> statsConsultantGet(from, to, type, format)
...
try {
    final response = api.statsConsultantGet(from, to, type, format);
    print(response);
} catch on DioException (e) {
...
# **statsProviderGet**
> StatsProviderGet200Response statsProviderGet(from, to, type, format)
...
try {
    final response = api.statsProviderGet(from, to, type, format);
    print(response);
} catch on DioException (e) {
