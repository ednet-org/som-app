...
class Model extends Entity<Model> {
  ...
  List<Concept> get orderedEntryConcepts {
    ...
    for (var c = 0; c < 9; c++) {
      ...
      assert(
        sameExternalCountConcepts.length ==
  ...
  /// Alias for concepts collection to match domain modeling API
  Concepts get entities => concepts;

  /// Get model name (alias for code)
  ...
  /// Create model from Map representation
  static Model fromMap(Map<String, dynamic> map, Domain domain) {
    final model = Model(domain, map['code'] ?? 'default');
